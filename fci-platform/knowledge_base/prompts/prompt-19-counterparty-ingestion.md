...
3. For the **top 5 risk-flagged counterparties by transaction volume**, provide: UID, source(s) (IT/BP/P2P), volume, nationality/residence, offboard status if offboarded (including reason), and a brief list of flags (e.g., "3 LE requests, 2 ICR cases, blocked — account/trade/withdraw — Reason: AML investigation"). **Always include both block types AND block reasons when present.** If a counterparty is offboarded and fully blocked, state this explicitly. Do not expand LE case details or list individual case reference numbers for these top entries — the flag summary is sufficient, but block reasons must always be included.
