...
async def download_c360_csv(case_id: str, current_user: dict = Depends(get_current_user)):
    """Download the Elliptic batch screening CSV generated by C360 processing."""
    case = await _get_case_or_404(case_id)
    c360 = case.get('sections', {}).get('c360', {})

    csv_content = c360.get('csv_content')
    if not csv_content:
        raise HTTPException(status_code=404, detail='No CSV available. C360 processing may not have extracted any wallet addresses.')

    csv_filename = c360.get('csv_filename', 'elliptic_screening.csv')
